...
Our attack is not applicable to every use of the ATECC508A. In particular, we are not able to retrieve a data slot declared as a P256 private key. Many IoT applications relies on those P256 keys and can still be considered somewhat safe. Additionally, this circuit is now considered deprecated and use of its successor - the ATECC508 - is recommended by Microchip for new designs.
